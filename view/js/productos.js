$(document).ready(function () {
  ...
  fetch("../controller/obtenerCategorias.php")  // Cambia esto por la ruta real a tu PHP
      .then(response => {
        if (!response.ok) {
...
$('.btn.btn-success').on('click', function () {
  ...
  $.ajax({
    url: '../controller/filtrarProductos.php', // asegúrate de que esta ruta es correcta
    method: 'POST',
    data: filtros,
    dataType: 'json',
  ...
  function cargarProductos() {
    $.ajax({
      url: '../controller/traerProductosBuscados.php',
      method: 'GET',
      dataType: 'json',
      success: function (productos) {
  ...
  productos.forEach(p => {
    const estrellas = generarEstrellas(p.rating || 0); // por si calificacion es null o undefined
